...
export const HowToBuy = ({ templateData }: { templateData: TemplateFormData }) => {
  ...
  return (
    <section id="how-to-buy" className={`w-full px-4 py-12`} style={{ backgroundColor: templateData.primaryColor }}>
        ...
        <div className="flex flex-col gap-4">
          {howToBuyItems.map((item, index) => (
            <div key={index} className="relative group">
              ...
              {/* How to buy item */}
              <div className="bg-white rounded-xl py-4 sm:py-5 border border-solid relative z-10" style={{ borderColor: templateData.borderColor }}>
                <div className="px-4 sm:px-6 py-2 sm:py-4 flex flex-col items-center gap-2 sm:gap-3 w-full">
                  <span className="flex-shrink-0 flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full text-white font-bold text-sm sm:text-base" style={{ backgroundColor: templateData.accentColor, color: templateData.headingColor }}>
                    {index + 1}
                  </span>
                  <span className="font-bold text-base sm:text-lg text-center break-words w-full max-w-full overflow-hidden" style={{ color: templateData.textColor }}>
                    {item.text}
                  </span>
                </div>
              </div>
